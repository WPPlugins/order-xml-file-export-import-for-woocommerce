...
class OrderImpExpXML_OrderImport extends WP_Importer {
    ...
    public function import($import_type, $import_decision) {
        ...
        $this->log->add('xml-import', '---');
        $this->log->add('xml-import', __('Processing orders.', 'wf_order_import_export_xml'));
        $merging = 1;
        $record_offset = 0;
        switch($import_type)
        {
        ...
        $this->log->add('xml-import', __('Finished processing Orders.', 'wf_order_import_export_xml'));
    ...
    public function import_start($file) {
        ...
        $this->log->add('xml-import', '---[ New Import ] PHP Memory: ' . $memory . ', WP Memory: ' . $wp_memory);
        $this->log->add('xml-import', __('Parsing order XML.', 'wf_order_import_export_xml'));

        $this->parser = new OrderImpExpXML_Parser('shop_order');

        $this->parsed_data = $this->parser->parse_data($file);

        $this->log->add('xml-import', __('Finished parsing order XML.', 'wf_order_import_export_xml'));
    ...
    private function process_orders($post,$import_type,$import_decision) {
        ...
        $this->log->add('xml-import', sprintf(__('> &#8220;%s&#8221;' . $out_updated_msg, 'wf_order_import_export_xml'), $post['order_number']), true);
        $this->log->add(sprintf(__('> Finished importing order %s', 'wf_order_import_export_xml'), $post['order_number']));


        $this->log->add(__('Finished processing orders.', 'wf_order_import_export_xml'));
